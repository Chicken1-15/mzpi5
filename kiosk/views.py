...
def free(request):
    name, password = get_user(request)
    url = 'http://adrastea.westus2.cloudapp.azure.com:3333/robots/tasks/add'
    form = KioskTargetParking(request.POST or None)
    if form['pozycja'].value() == '1':
        requests.post(url, auth=(name, password), json=[{"position": {
            "x": 0.00693932672489872, "y": 2.264171969771321, "z": 0}}])
    elif form['pozycja'].value() == '2':
        requests.post(url, auth=(name, password), json=[{"position": {
            "x": -0.09213410540779243, "y": -1.6140025714005297, "z": 0}}])
    elif form['pozycja'].value() == '3':
        requests.post(url, auth=(name, password), json=[{"position": {
            "x": -0.07355783688291417, "y": -0.9700252625380301, "z": 0}}])
    elif form['pozycja'].value() == '4':
        requests.post(url, auth=(name, password), json=[{"position": {
            "x": -1.6140025714005297, "y": -0.09213410540779243, "z": 0}}])
    if form.is_valid():
        form.save(commit=True)
        form = KioskTargetParking()
    context = {
        'form': form,
    }
